...
const BoardGroup: FC<IBoardGroupProps> = ({
	projectdata,
	groups,
	data,
	setData,
	...
	const CARD_COUNT = groups?.cards?.length || 0;
	const { id } = useParams();
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			await fetch('http://localhost:8080/api/issue/create', requestOptions);
	...
	const fetchMetaData = async () => {
		const usersData = await fetch(`http://localhost:8080/api/user/all/${userObject.orgId}`);
		let usersDataObject = await usersData.json();
		const peojectData = await fetch(`http://localhost:8080/api/project/${id}`);
		let peojectDataObject = await peojectData.json();
		const teamData = await fetch(`http://localhost:8080/api/team/${peojectDataObject?.teamId}`);
		const teamDataUsers = await teamData.json();
		const members = teamDataUsers.members;
		usersDataObject = usersDataObject?.filter((u: any) => members.includes(u.id));
